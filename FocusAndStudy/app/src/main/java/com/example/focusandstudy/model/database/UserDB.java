...
public class UserDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String createTableStatement = "CREATE TABLE " + USER_TABLE + "(" + COLUMN_ID + " INTEGER PRIMARY KEY AUTOINCREMENT,  " + COLUMN_USERNAME + " TEXT, " + COLUMN_EMAIL + " TEXT," + COLUMN_PASSWORD + " TEXT )";
        try {
    ...
    public boolean updateDB(User mUser){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();

        cv.put(COLUMN_USERNAME, mUser.getUsername());
        cv.put(COLUMN_EMAIL, mUser.getUsername());
